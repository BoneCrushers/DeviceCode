...
wav_file = "SoundFiles/Chicago_Amb_Airplane_Overhead_B-Format_44.1_16.wav"
...
def playAmbisonics(ambFile, ambvolume=.1, speed=1):
    ...
    SUBFORMAT_GUID = fileHeader[42:60]
    ...
    def audioCallbackAmbisonics(inData, frameCount, timeInfo, status):
        ...
        print("FrameCount", frameCount, "expected:Len(audioData)", frameCount * FRAME_SIZE_IN_BYTES, "len(audioData)",
              len(audioData))
        ...
        for i in range(frameCount):
            workingData = [int.from_bytes(audioData[adByteStart + x:adByteStart + (x + 2)],
                                          byteorder='little') for x in range(0, 8, 2)]
            # workingData = [
